...
<font class="text-primary"><strong>Desbloquear Email: </strong></font><br>    
    ...
    <?php
        ...
        $consulta = mysqli_query($datos_conexion, "SELECT * FROM `emailsBloqueados` INNER JOIN `usuarios` ON usuarios.idUsuario = emailsBloqueados.idUsuarioQueBloqueo WHERE (emailsBloqueados.idEmailsBloqueados = $idEmailsBloqueados)");
        ...
        while ($listar = mysqli_fetch_assoc($consulta)) {// 
            ...
            /*
            . "' class='btn btn-danger'>Desbloquear </a> ->> " . $listar['emailBloqueado'] . "<strong> / </strong>" . $listar['motivoBloqueo'] . " <strong> / </strong> <font class='text-success'>" . $listar['fechaBloqueo'] . "</font> <strong> / </strong> Bloqueado por: <font class='text-primary'>" . $listar['nombreDeUsuario'] . "</font>";
            echo "</td>";
            */
        }
    ...
    <h4><p class="text-danger">Cuidado si continúa se desbloquará la cuenta de correo electrónico</p></h4>

    <?php
        echo "<a href='update_desbloquear_email.php?param=" . $idEmailsBloqueados . "&idusuario=" . $idusuarioget . "' class='btn btn-success'>Desbloquear</a>";
    ?>
